...
class StableDiffusion3RewardPipeline(DiffusionPipeline, SD3LoraLoaderMixin, FromSingleFileMixin, SD3IPAdapterMixin):
    r"""
    Args:
    ...
    def __init__(
        self,
        transformer: SD3Transformer2DRewardModel,
        scheduler: FlowMatchEulerDiscreteScheduler,
        vae: AutoencoderKL,
        text_encoder: CLIPTextModelWithProjection,
        tokenizer: CLIPTokenizer,
        ...
        self.scheduler_copy = copy.deepcopy(scheduler)
        ...
        self._preprocess_image = ImageCropAndResize(max_pixels=max_pixels)
    ...
    def get_sigmas(self, timesteps, n_dim=4, dtype=torch.float32, device='cuda'):
        sigmas = self.scheduler_copy.sigmas.to(device=device, dtype=dtype)
        schedule_timesteps = self.scheduler_copy.timesteps.to(device)
    ...
    def prepare_latents(
        ...
        for idx, image in enumerate(images):
            ...
            if isinstance(image, str):
                images[idx] = self._preprocess_image(Image.open(image).convert("RGB"))
            if isinstance(image, Image.Image):
                images[idx] = self._preprocess_image(image.convert("RGB"))
        images = torch.stack(images, dim=0).to(device=device, dtype=dtype)
    ...
    def __call__(
        ...
        if timestep is None:
            timestep = timesteps[current_inference_step]
        else:
            timestep = torch.tensor(timestep, dtype=torch.int32, device=device)
            indices = (self.scheduler_copy.config.num_train_timesteps - timestep).long()
            timestep = self.scheduler_copy.timesteps[indices].to(device=device)
