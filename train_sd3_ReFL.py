...
def main():
    ...
    def run_validation(transformer, vae, text_encoders, tokenizers, val_dataloader, weight_dtype, global_step):
        ...
        with torch.no_grad():
            for step, batch in enumerate(tqdm(val_dataloader, desc="Validating", disable=not accelerator.is_local_main_process)):
                ...
                generator = torch.Generator(device=accelerator.device).manual_seed(args.seed + step if args.seed else 42)
                win_noise = torch.randn(win_model_input.shape, generator=generator, device=accelerator.device, dtype=win_model_input.dtype)
                lose_noise = torch.randn(lose_model_input.shape, generator=generator, device=accelerator.device, dtype=win_model_input.dtype)

                indices = torch.full((bsz,), 999, dtype=torch.long)
                ...
                acc = ((score_win - score_lose) > 0).sum().float() / inside_term.size(0)
                avg_acc = accelerator.gather(acc).mean().item()
                val_acc += avg_acc
                total_samples += 1

        final_acc = val_acc / total_samples
        
        logger.info(f"Step {global_step}: Val Acc: {final_acc:.4f}")
        accelerator.log({"val_acc": final_acc}, step=global_step)
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if accelerator.sync_gradients:
                ...
                if global_step % args.validation_steps == 0:
                    run_validation(transformer, vae, text_encoders, tokenizers, val_dataloader, weight_dtype, global_step)
